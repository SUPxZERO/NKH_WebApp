...
const SPICE_LEVELS = [
  { level: 0, label: 'Not Spicy', icon: '🥛', color: 'text-gray-500' },
  { level: 1, label: 'Mild', icon: '🌶️', color: 'text-green-500' },
  { level: 2, label: 'Medium', icon: '🌶️🌶️', color: 'text-yellow-500' },
  { level: 3, label: 'Hot', icon: '🌶️🌶️🌶️', color: 'text-orange-500' },
  { level: 4, label: 'Very Hot', icon: '🔥', color: 'text-red-500' },
  { level: 5, label: 'Extreme', icon: '💀', color: 'text-red-700' },
];

const DIETARY_ICONS: Record<string, { icon: React.ReactNode; label: string; color: string; bg: string }> = {
  vegetarian: { icon: <Leaf className="w-4 h-4" />, label: 'Vegetarian', color: 'text-green-600', bg: 'bg-green-100 dark:bg-green-900/30' },
  vegan: { icon: <Leaf className="w-4 h-4" />, label: 'Vegan', color: 'text-green-700', bg: 'bg-green-200 dark:bg-green-900/50' },
  'gluten-free': { icon: '🌾', label: 'Gluten Free', color: 'text-amber-600', bg: 'bg-amber-100 dark:bg-amber-900/30' },
  keto: { icon: '🥑', label: 'Keto Friendly', color: 'text-emerald-600', bg: 'bg-emerald-100 dark:bg-emerald-900/30' },
  halal: { icon: '🕌', label: 'Halal', color: 'text-blue-600', bg: 'bg-blue-100 dark:bg-blue-900/30' },
  kosher: { icon: '✡️', label: 'Kosher', color: 'text-indigo-600', bg: 'bg-indigo-100 dark:bg-indigo-900/30' },
  'dairy-free': { icon: '🥛', label: 'Dairy Free', color: 'text-sky-600', bg: 'bg-sky-100 dark:bg-sky-900/30' },
  'nut-free': { icon: '🥜', label: 'Nut Free', color: 'text-orange-600', bg: 'bg-orange-100 dark:bg-orange-900/30' },
};
          ...
          >
            ...
            {isLoading && (
              <div className="flex flex-col items-center justify-center py-20 px-6">
                ...
                <p className="text-neutral-600 dark:text-neutral-400">Loading details...</p>
              </div>
            )}
                  ...
                  <div className="p-6">
                    ...
                    <div className="mb-6">
                      ...
                      <div className={cn('inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium', availabilityStyle.bg, availabilityStyle.text)}>
                        ...
                        <span className="capitalize">{food.availability_status?.replace('_', ' ') || 'Available'}</span>
                    ...
                    <div className="flex flex-wrap gap-3 mb-6">
                      ...
                      {food.spice_level !== undefined && food.spice_level > 0 && (
                        <div className={cn('flex items-center gap-2 px-3 py-2 rounded-xl bg-neutral-100 dark:bg-neutral-800', spiceInfo.color)}>
                          ...
                          <span className="text-sm font-medium">{spiceInfo.label}</span>
                        </div>
                      )}
                    </div>
                    ...
                    {(food.dietary_tags?.length || food.dietary_restrictions?.length) && (
                      <div className="flex flex-wrap gap-2 mb-6">
                        {(food.dietary_tags || food.dietary_restrictions || []).map((tag) => {
                          const tagInfo = DIETARY_ICONS[tag.toLowerCase()];
                    ...
                    <div className="flex border-b border-neutral-200 dark:border-neutral-700 mb-4">
                      {['overview', 'nutrition', 'recipe'].map((tab) => (
                        <button
                          key={tab}
                          ...
                          {food.nutrition ? (
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                              {food.nutrition.calories !== undefined && (
                                <NutritionCard label="Calories" value={food.nutrition.calories} unit="kcal" color="orange" />
                              )}
                              {food.nutrition.protein !== undefined && (
                                <NutritionCard label="Protein" value={food.nutrition.protein} unit="g" color="blue" />
                              )}
                              {food.nutrition.carbs !== undefined && (
                                <NutritionCard label="Carbs" value={food.nutrition.carbs} unit="g" color="yellow" />
                              )}
                              {food.nutrition.fat !== undefined && (
                                <NutritionCard label="Fat" value={food.nutrition.fat} unit="g" color="red" />
                              )}
                              {food.nutrition.fiber !== undefined && (
                                <NutritionCard label="Fiber" value={food.nutrition.fiber} unit="g" color="green" />
                              )}
                              {food.nutrition.sodium !== undefined && (
                                <NutritionCard label="Sodium" value={food.nutrition.sodium} unit="mg" color="purple" />
                              )}
                              {food.nutrition.sugar !== undefined && (
                                <NutritionCard label="Sugar" value={food.nutrition.sugar} unit="g" color="pink" />
                              )}
                            </div>
                          ) : food.calories ? (
                            <div className="text-center py-8">
                              ...
                              <p className="text-neutral-500">Calories per serving</p>
                            </div>
                          ) : (
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                      ...
                                      <p className="font-medium text-neutral-900 dark:text-white">
                                        {food.recipe.prep_time_minutes} min
                                      </p>
                                    </div>
                                  </div>
                                )}
                                {food.recipe.cook_time_minutes && (
                                  <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                      ...
                                      <p className="font-medium text-neutral-900 dark:text-white">
                                        {food.recipe.cook_time_minutes} min
                                      </p>
                                    </div>
                                  </div>
                                )}
