...
export default function Dashboard() {
  ...
  return (
    <CustomerLayout>
      ...
      >
        {/* HERO SECTION */}
        <div className="relative rounded-3xl overflow-hidden shadow-2xl shadow-fuchsia-500/10">
          ...
          <div className="relative z-10 p-8 md:p-12 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <motion.div
              ...
              className="bg-white/15 backdrop-blur-xl border border-white/25 rounded-2xl p-6 relative overflow-hidden"
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {[
            ...
            { label: 'Points Earned', value: `+${stats?.points_earned_this_month || 0}`, icon: Zap, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-100 dark:bg-amber-900/30', border: 'border-amber-200 dark:border-amber-800/50', sub: 'This Month' },
            ...
            <motion.div
              key={idx}
              variants={itemVariants}
              className="relative rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-5 md:p-6 transition-all hover:shadow-lg hover:shadow-gray-200/50 dark:hover:shadow-gray-900/50 hover:border-fuchsia-200 dark:hover:border-fuchsia-700"
            >
              <div className="flex items-center justify-between mb-4">
                <div className={cn("p-2.5 md:p-3 rounded-xl border", stat.bg, stat.border)}>
                  <stat.icon className={cn("w-5 h-5 md:w-6 md:h-6", stat.color)} />
                </div>
                {stat.sub && <span className="text-[10px] md:text-xs font-semibold px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">{stat.sub}</span>}
              </div>
        ...
        >
          <button
            ...
            className="w-full md:w-auto group relative overflow-hidden rounded-2xl bg-gradient-to-r from-fuchsia-600 via-purple-600 to-fuchsia-600 bg-[length:200%_100%] p-6 shadow-xl shadow-fuchsia-500/30 border border-fuchsia-500/20 hover:bg-[position:100%_0] hover:shadow-2xl hover:shadow-fuchsia-500/40 hover:-translate-y-0.5 transition-all duration-300"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  <h3 className="text-lg font-bold text-white">Loyalty Rewards Program</h3>
                  <p className="text-sm text-white/80">View your tier, earn points, and unlock rewards</p>
                </div>
              </div>
        ...
        <BrandDivider variant="dots" className="opacity-30 my-8" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Sidebar - Reservations & Favorites */}
          <div className="space-y-6">
            {/* Upcoming Reservations */}
            <div className="rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-gray-900 dark:text-white">Reservations</h3>
                <button onClick={() => window.location.href = '/customer/reservations'} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors"><ArrowRight className="w-4 h-4 text-gray-500 dark:text-gray-400" /></button>
              </div>
              ...
              {reservationsLoading ? (
                ...
                <div className="space-y-3">
                  {customerReservations.slice(0, 3).map(res => (
                    <div key={res.id} className="p-3 rounded-xl bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-600 flex justify-between items-center group hover:border-fuchsia-300 dark:hover:border-fuchsia-700 transition-colors">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 flex items-center justify-center font-bold text-xs border border-orange-200 dark:border-orange-800/50">
                          {new Date(res.reserved_for).getDate()}
                        </div>
                        <div>
                          <div className="text-sm font-bold text-gray-900 dark:text-white">{new Date(res.reserved_for).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                          <div className="text-xs text-gray-500 dark:text-gray-400">{res.guest_count} Guests</div>
                        </div>
                      </div>
                      {res.status === 'pending' && (
                        <button onClick={() => handleCancelReservation(res.id)} className="text-xs text-red-500 dark:text-red-400 hover:underline">Cancel</button>
                      )}
                    </div>
                  ))}
                </div>
                ...
                <div className="text-center py-6">
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">No upcoming reservations</p>
                  <button onClick={() => window.location.href = '/customer/reservations'} className="text-xs font-bold text-fuchsia-600 dark:text-fuchsia-400 uppercase tracking-wider hover:underline">Book a Table</button>
                </div>
              )}
            </div>
            ...
            {/* Favorites */}
            <div className="rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-5">
              <h3 className="font-bold text-gray-900 dark:text-white mb-4">Your Favorites</h3>
              {profile?.favorite_items && profile.favorite_items.length > 0 ? (
                <div className="space-y-3">
                  {profile.favorite_items.slice(0, 4).map(item => (
                    <div
                      ...
                      className="flex items-center gap-3 p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer group"
                    >
                      <img src={item.image_path || '/images/default-food.png'} alt={item.name} className="w-12 h-12 rounded-lg object-cover border border-gray-200 dark:border-gray-600" />
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-sm text-gray-900 dark:text-white truncate group-hover:text-fuchsia-600 dark:group-hover:text-fuchsia-400 transition-colors">{item.name}</h4>
                        <p className="text-xs text-gray-500 dark:text-gray-400">${Number(item.price).toFixed(2)}</p>
                      </div>
                      <div className="w-8 h-8 rounded-full bg-fuchsia-100 dark:bg-fuchsia-900/30 flex items-center justify-center text-fuchsia-500 dark:text-fuchsia-400 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-4">Make some orders to see favorites!</p>
              )}
            </div>
          </div>
        </div>
