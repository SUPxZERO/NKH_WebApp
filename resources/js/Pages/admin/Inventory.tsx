...
const InventoryStatsRibbon = ({ stats }: { stats: any }) => (
  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <div className="bg-white/5 border border-white/10 rounded-xl p-4 backdrop-blur-sm">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-2xl font-bold text-emerald-400 mt-1">${Number(stats.totalValue).toFixed(2)}</p>
        </div>
    ...
    <div className="bg-white/5 border border-white/10 rounded-xl p-4 backdrop-blur-sm">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-gray-400 text-xs uppercase tracking-wider font-medium">Expiring Soon</p>
          <p className="text-2xl font-bold text-red-400 mt-1">{stats.expiringSoon}</p>
        </div>
    ...
    <div className="bg-white/5 border border-white/10 rounded-xl p-4 backdrop-blur-sm">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-2xl font-bold text-blue-400 mt-1">{stats.totalItems}</p>
        </div>
      ...
      <div className="min-h-screen bg-slate-900 p-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-400 mt-1">Stock tracking and management</p>
          </div>
            ...
            {isLoading ? (
              ...
              <motion.div key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                <div className="col-span-2">
                  <Badge className="bg-blue-500/10 text-blue-400 border-blue-500/20 flex items-center gap-1 w-fit">
                    <MapPin size={10} /> {item.location.name}
                  </Badge>
                </div>
                <div className="col-span-2">
                  <div className="text-white font-bold">{Number(item.quantity).toFixed(3)} <span className="text-gray-400 text-xs font-normal">{item.ingredient.unit?.code}</span></div>
                ...
                <div className="col-span-2 text-sm">
                  {item.expiration_date ? (
                    <span className={cn(
                      ...
                      isExpired(item.expiration_date) ? "text-red-400 font-bold" :
                        isExpiringSoon(item.expiration_date) ? "text-amber-400 font-medium" : "text-gray-300"
                    )}>
                ...
                <div className="col-span-1 flex justify-end">
                  <Button size="sm" variant="secondary" onClick={() => { setSelectedItem(item); setOpenMovements(true); }} className="h-8 w-8 p-0 border-white/10"><History size={14} /></Button>
                </div>
              </motion.div>
            ))}
          </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-gray-300 text-sm mb-1 block">From Location</label>
          <select required className="w-full bg-slate-950 border border-white/10 rounded-lg p-2 text-white"
        ...
        <div>
          <label className="text-gray-300 text-sm mb-1 block">To Location</label>
          <select required className="w-full bg-slate-950 border border-white/10 rounded-lg p-2 text-white"
      ...
      <div>
        <label className="text-gray-300 text-sm mb-1 block">Ingredient</label>
        <select required className="w-full bg-slate-950 border border-white/10 rounded-lg p-2 text-white"
      ...
      <Input label="Quantity" type="number" step="0.001" required value={formData.quantity} onChange={e => setFormData({ ...formData, quantity: e.target.value })} className="bg-slate-950 border-white/10" />
      <Input label="Notes" value={formData.notes} onChange={e => setFormData({ ...formData, notes: e.target.value })} className="bg-slate-950 border-white/10" />
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Input label="Quantity" type="number" step="0.001" required value={formData.quantity} onChange={e => setFormData({ ...formData, quantity: e.target.value })} className="bg-slate-950 border-white/10" />
        <div>
          <label className="text-gray-300 text-sm mb-1 block">Reason</label>
          <select required className="w-full bg-slate-950 border border-white/10 rounded-lg p-2 text-white"
        ...
        <div key={m.id} className="bg-white/5 p-3 rounded border border-white/10 flex justify-between items-center">
          <div>
            <div className="text-white font-medium capitalize flex items-center gap-2">
              {m.type === 'purchase_received' && <Package size={14} className="text-blue-400" />}
              {m.type === 'wastage' && <Trash2 size={14} className="text-red-400" />}
              {m.type === 'transfer_in' && <ArrowRightLeft size={14} className="text-green-400" />}
              {m.type.replace('_', ' ')}
            </div>
          ...
          <div className={`font-bold ${m.quantity > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
