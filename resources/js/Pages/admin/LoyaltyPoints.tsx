  ...
  return (
    <div className="-mx-3 sm:mx-0 px-3 sm:px-0 overflow-x-auto scrollbar-hide mb-4 sm:mb-6">
      <div className="flex sm:grid sm:grid-cols-4 gap-2 sm:gap-4 min-w-max sm:min-w-0">
        <StatCard title={t('admin.marketing.loyalty.stats.earned')} value={stats.earned.toLocaleString()} icon={TrendingUp} color="emerald" index={0} />
        <StatCard title={t('admin.marketing.loyalty.stats.redeemed')} value={stats.redeemed.toLocaleString()} icon={Gift} color="blue" index={1} />
        <StatCard title={t('admin.marketing.loyalty.stats.customers')} value={stats.activeCustomers} icon={Users} color="purple" index={2} />
        <StatCard title={t('admin.marketing.loyalty.stats.txns')} value={stats.transactions} icon={Star} color="amber" index={3} />
      </div>
    </div>
  );
};
...
export default function LoyaltyPoints() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { toastSuccess(t('admin.marketing.loyalty.messages.created')); closeModal(); qc.invalidateQueries({ queryKey: ['loyalty-points'] }); qc.invalidateQueries({ queryKey: ['loyalty-stats'] }); },
    onError: (err: any) => toastError(err.response?.data?.message || t('admin.marketing.loyalty.messages.failed'))
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number, data: any }) => apiPut(`/api/admin/loyalty-points/${id}`, data),
    onSuccess: () => { toastSuccess(t('admin.marketing.loyalty.messages.updated')); closeModal(); qc.invalidateQueries({ queryKey: ['loyalty-points'] }); },
    onError: (err: any) => toastError(err.response?.data?.message || t('admin.marketing.loyalty.messages.failed'))
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiDelete(`/api/admin/loyalty-points/${id}`),
    onSuccess: () => { toastSuccess(t('admin.marketing.loyalty.messages.deleted')); qc.invalidateQueries({ queryKey: ['loyalty-points'] }); },
    onError: (err: any) => toastError(err.response?.data?.message || t('admin.marketing.loyalty.messages.failed'))
  });
  ...
  return (
    <AdminLayout>
      <div className="min-h-screen bg-background p-3 sm:p-4 md:p-6 overflow-x-hidden">
        ...
        <div className="relative flex items-center justify-between gap-3 mb-4 sm:mb-6 md:mb-8">
          <div className="min-w-0">
            ...
            >
              {t('admin.marketing.loyalty.title')}
            </motion.h1>
            <p className="text-muted-foreground text-xs sm:text-sm mt-0.5 hidden sm:block">{t('admin.marketing.loyalty.subtitle')}</p>
          </div>
          <Button onClick={() => { closeModal(); setFormData({ ...formData, occurred_at: new Date().toISOString().slice(0, 16) }); setOpenCreate(true); }} className="h-9 sm:h-10 px-2 sm:px-4 text-xs sm:text-sm flex-shrink-0 bg-purple-600 hover:bg-purple-700">
            ...
            <span className="hidden sm:inline">{t('admin.marketing.loyalty.add_button')}</span>
          </Button>
        </div>
        ...
        >
          <div className="flex gap-2 sm:gap-4">
            <div className="relative flex-1 min-w-0">
              ...
              <Input placeholder={t('admin.marketing.loyalty.filters.search')} value={search} onChange={(e) => setSearch(e.target.value)}
                className="pl-9 h-10 text-sm" variant="filled" />
            </div>
            <div className="flex gap-1 sm:gap-2 overflow-x-auto scrollbar-hide">
              {[
                { key: 'all', label: t('admin.marketing.loyalty.filters.type.all'), mobile: t('admin.marketing.loyalty.filters.type.all') },
                { key: 'earn', label: t('admin.marketing.loyalty.filters.type.earn'), mobile: '+' },
                { key: 'redeem', label: t('admin.marketing.loyalty.filters.type.redeem'), mobile: '-' },
                { key: 'adjust', label: t('admin.marketing.loyalty.filters.type.adjust'), mobile: '±' }
        ...
        >
          ...
          <div className="grid grid-cols-12 gap-4 p-4 border-b border-border/50 bg-gradient-to-r from-purple-500/10 via-fuchsia-500/5 to-purple-500/10">
            <div className="col-span-3 text-xs font-bold text-foreground uppercase tracking-wider">{t('admin.marketing.loyalty.table.headers.customer')}</div>
            <div className="col-span-2 text-xs font-bold text-foreground uppercase tracking-wider">{t('admin.marketing.loyalty.table.headers.type')}</div>
            <div className="col-span-2 text-xs font-bold text-foreground uppercase tracking-wider">{t('admin.marketing.loyalty.table.headers.points')}</div>
            <div className="col-span-2 text-xs font-bold text-foreground uppercase tracking-wider">{t('admin.marketing.loyalty.table.headers.balance')}</div>
            <div className="col-span-2 text-xs font-bold text-foreground uppercase tracking-wider">{t('admin.marketing.loyalty.table.headers.date')}</div>
            <div className="col-span-1 text-xs font-bold text-foreground uppercase tracking-wider text-right">{t('admin.marketing.loyalty.table.headers.actions')}</div>
          </div>
            ...
            {isLoading ? (
              <div className="p-12 text-center">
                <div className="inline-flex items-center gap-3 text-muted-foreground">
                  ...
                  {t('admin.marketing.loyalty.table.loading')}
                </div>
              </div>
              ...
              <div className="p-12 text-center">
                ...
                <h3 className="text-foreground font-semibold">{t('admin.marketing.loyalty.table.empty.title')}</h3>
                <p className="text-muted-foreground text-sm mt-1">{t('admin.marketing.loyalty.table.empty.subtitle')}</p>
              </div>
              ...
              >
                ...
                <div className="col-span-1 flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-all">
                  ...
                  <Button size="sm" variant="ghost" onClick={() => confirm(t('admin.marketing.loyalty.messages.delete_confirm')) && deleteMutation.mutate(transaction.id)} className="h-7 w-7 p-0 hover:bg-red-500/20 hover:text-red-600"><Trash2 size={14} /></Button>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="md:hidden space-y-2">
          {isLoading ? (
            ...
            <div className="p-8 text-center bg-card/50 rounded-xl border border-border/50">
              ...
              <p className="text-muted-foreground text-sm">{t('admin.marketing.loyalty.table.empty.title')}</p>
            </div>
          ) : (
            transactionList.map((transaction: LoyaltyPoint, idx: number) => (
              <motion.div
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex gap-1 flex-shrink-0">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => confirm(t('admin.marketing.loyalty.messages.delete_confirm')) && deleteMutation.mutate(transaction.id)} className="h-8 w-8 p-0 hover:bg-red-500/20 hover:text-red-600">
                      <Trash2 size={14} />
                    </Button>
                  </div>
                </div>
      ...
      <Modal open={openCreate || openEdit} onClose={closeModal} title={editingTransaction ? t('admin.marketing.loyalty.form.edit_title') : t('admin.marketing.loyalty.form.create_title')} size="lg">
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('admin.marketing.loyalty.form.fields.customer')}</label>
              <select value={formData.customer_id} onChange={(e) => setFormData({ ...formData, customer_id: e.target.value })} required
                ...
                <option value="">{t('admin.marketing.loyalty.form.placeholders.search_customer')}</option>
                {customers?.data?.map((c: any) => <option key={c.id} value={c.id}>{c.user?.name}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('admin.marketing.loyalty.form.fields.type')}</label>
              <select value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value as any })}
                className="w-full h-10 bg-white dark:bg-slate-950 border border-gray-300 dark:border-white/10 rounded-lg px-3 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition-all">
                <option value="earn">{t('admin.marketing.loyalty.form.options.earn')}</option>
                <option value="redeem">{t('admin.marketing.loyalty.form.options.redeem')}</option>
                <option value="adjust">{t('admin.marketing.loyalty.form.options.adjust')}</option>
              </select>
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <Input label={t('admin.marketing.loyalty.form.fields.points')} type="number" value={formData.points} onChange={(e) => setFormData({ ...formData, points: e.target.value })} required className="h-10 text-sm" placeholder={formData.type === 'adjust' ? t('admin.marketing.loyalty.form.placeholders.points_adjust') : t('admin.marketing.loyalty.form.placeholders.points_earn')} />
            <Input label={t('admin.marketing.loyalty.form.fields.date')} type="datetime-local" value={formData.occurred_at} onChange={(e) => setFormData({ ...formData, occurred_at: e.target.value })} required className="h-10 text-sm" />
          </div>
          <div className="hidden sm:block">
            <label className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('admin.marketing.loyalty.form.fields.notes')}</label>
            <textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} rows={2}
              className="w-full bg-white dark:bg-slate-950 border border-gray-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white placeholder:text-gray-500 focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition-all" placeholder={t('admin.marketing.loyalty.form.placeholders.notes')} />
          </div>
