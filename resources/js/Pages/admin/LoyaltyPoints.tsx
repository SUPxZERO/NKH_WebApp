...
const LoyaltyStatsRibbon = ({ stats }: { stats: any }) => (
  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <StatCard title="Total Earned" value={stats.earned.toLocaleString()} icon={TrendingUp} color="emerald" index={0} />
    <StatCard title="Redeemed" value={stats.redeemed.toLocaleString()} icon={Gift} color="blue" index={1} />
    <StatCard title="Active Customers" value={stats.activeCustomers} icon={Users} color="purple" index={2} />
    <StatCard title="Transactions" value={stats.transactions} icon={Star} color="amber" index={3} />
  </div>
);
    ...
    <AdminLayout>
      <div className="min-h-screen bg-background p-6">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Customer rewards and point transactions</p>
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-12 gap-4 p-4 border-b border-border/50 bg-gradient-to-r from-purple-500/10 via-fuchsia-500/5 to-purple-500/10">
            ...
            <div className="col-span-2 text-xs font-bold text-foreground uppercase tracking-wider">Balance After</div>
            <div className="col-span-2 text-xs font-bold text-foreground uppercase tracking-wider">Date / Order</div>
          ...
          <div className="divide-y divide-border/30">
            {isLoading ? (
              ...
              <div className="p-12 text-center">
                ...
                <p className="text-muted-foreground text-sm mt-1">Create your first loyalty point transaction to get started</p>
              </div>
              ...
              >
                ...
                <div className="col-span-2 text-sm">
                  ...
                  {transaction.order_id && <div className="text-xs text-purple-600 dark:text-purple-400">Order #{transaction.order_id}</div>}
                </div>
                <div className="col-span-1 flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-all">
                  <Button size="sm" variant="ghost" onClick={() => handleEdit(transaction)} className="h-7 w-7 p-0 hover:bg-purple-500/20 hover:text-purple-600 dark:hover:text-purple-400"><Edit size={14} /></Button>
                  <Button size="sm" variant="ghost" onClick={() => confirm('Delete?') && deleteMutation.mutate(transaction.id)} className="h-7 w-7 p-0 hover:bg-red-500/20 hover:text-red-600 dark:hover:text-red-400"><Trash2 size={14} /></Button>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <Modal open={openCreate || openEdit} onClose={closeModal} title={editingTransaction ? 'Edit Transaction' : 'New Transaction'} size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={formData.customer_id} onChange={(e) => setFormData({ ...formData, customer_id: e.target.value })} required
                className="w-full bg-white dark:bg-slate-950 border border-gray-300 dark:border-white/10 rounded-lg px-3 py-2 text-gray-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition-all">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input label="Points" type="number" min="1" value={formData.points} onChange={(e) => setFormData({ ...formData, points: e.target.value })} required />
            <Input label="Date & Time" type="datetime-local" value={formData.occurred_at} onChange={(e) => setFormData({ ...formData, occurred_at: e.target.value })} required />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Notes</label>
            <textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} rows={3}
              className="w-full bg-white dark:bg-slate-950 border border-gray-300 dark:border-white/10 rounded-lg px-3 py-2 text-gray-900 dark:text-white placeholder:text-gray-500 focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition-all" placeholder="Optional notes..." />
          </div>
