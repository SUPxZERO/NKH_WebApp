...
const FloorStatsRibbon = ({ stats }: { stats: any }) => (
  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <StatCard title="Total Floors" value={stats.total} icon={Building} color="purple" index={0} />
    <StatCard title="Active" value={stats.active} icon={CheckCircle} color="emerald" index={1} />
    <StatCard title="Total Tables" value={stats.tables} icon={Grid3X3} color="blue" index={2} />
    <StatCard title="Total Capacity" value={stats.capacity} icon={Users} color="amber" index={3} />
  </div>
);
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <motion.h1
              ...
              className="text-3xl font-bold bg-gradient-to-r from-purple-500 via-violet-500 to-purple-500 bg-clip-text text-transparent"
            >
              Floors
            </motion.h1>
            <p className="text-muted-foreground mt-1">Manage restaurant layout</p>
          </div>
        ...
        >
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <select value={locationFilter} onChange={(e) => setLocationFilter(e.target.value)}
              className="bg-secondary border border-border rounded-xl px-4 py-2 text-foreground focus:border-purple-500 outline-none transition-colors">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                required
                className="w-full bg-background border border-border rounded-lg px-3 py-2 text-foreground focus:border-purple-500 outline-none transition-colors"
              >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-center h-full pt-6">
              <label className="flex items-center gap-2 cursor-pointer text-foreground">
                <input type="checkbox" checked={formData.is_active} onChange={(e) => setFormData({ ...formData, is_active: e.target.checked })} className="rounded bg-background border-border" /> Active
              </label>
            </div>
          </div>
