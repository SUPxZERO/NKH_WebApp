...
class DashboardController extends Controller
{
    ...
    private function getStats(): JsonResponse
    {
        try {
            ...
            // Menu Performance
            $popularItems = MenuItem::withCount([
                'orderItems' => function ($query) use ($thisMonth) {
            ...
            $orderStatusDistribution = Order::join('order_statuses', 'orders.order_status_id', '=', 'order_statuses.id')
                ->select('order_statuses.code as status', DB::raw('count(*) as count'))
                ->whereDate('orders.created_at', '>=', $thisMonth)
