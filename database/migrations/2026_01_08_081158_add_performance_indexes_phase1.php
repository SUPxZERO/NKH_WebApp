...
return new class extends Migration
{
     ...
     * Run the migrations.
     * 
     * Phase 1 Performance Indexes - Fixed to skip existing
    ...
    public function up(): void
    {
        ...
        Schema::table('orders', function (Blueprint $table) use ($indexExists) {
            if (!$indexExists('orders', 'idx_orders_location_created')) {
                $table->index(['location_id', 'created_at'], 'idx_orders_location_created');
            }
            if (!$indexExists('orders', 'idx_orders_location_status')) {
                $table->index(['location_id', 'status'], 'idx_orders_location_status');
            }
            if (!$indexExists('orders', 'idx_orders_customer_created')) {
        ...
        if (Schema::hasTable('order_items')) {
            Schema::table('order_items', function (Blueprint $table) use ($indexExists) {
                if (!$indexExists('order_items', 'idx_order_items_order')) {
        ...
        if (Schema::hasTable('invoices')) {
            Schema::table('invoices', function (Blueprint $table) use ($indexExists) {
                if (!$indexExists('invoices', 'idx_invoices_created')) {
                    $table->index(['created_at'], 'idx_invoices_created');
                }
                if (Schema::hasColumn('invoices', 'amount_due') && !$indexExists('invoices', 'idx_invoices_amount_due')) {
        ...
        if (Schema::hasTable('payments')) {
            Schema::table('payments', function (Blueprint $table) use ($indexExists) {
                if (!$indexExists('payments', 'idx_payments_invoice_status')) {
