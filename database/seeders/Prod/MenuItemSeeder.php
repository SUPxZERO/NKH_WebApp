...
class MenuItemSeeder extends Seeder
{
    ...
    private function createItem($location, $category, $data, $displayOrder): array
    {
        ...
        return [
            ...
            'featured_order' => $data[9] ? $displayOrder : 999,
            'is_active' => true,
    ...
    private function getAllergens($slug): array {
        $allergens = [];
        if (str_contains($slug, 'peanut') || str_contains($slug, 'satay')) $allergens[] = 'peanuts';
        if (str_contains($slug, 'shrimp') || str_contains($slug, 'prawn') || str_contains($slug, 'crab') || str_contains($slug, 'seafood')) $allergens[] = 'shellfish';
        return $allergens;
    }

    private function getDietaryTags($slug): array {
        $tags = [];
        if (str_contains($slug, 'vegetable') || str_contains($slug, 'tofu')) $tags[] = 'vegetarian';
        return $tags;
    }
